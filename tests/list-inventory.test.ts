...
describe("list_inventory", () => {
  ...
  describe("registration", () => {
    it("registers under the expected name with optional input schema", async () => {
      const mod = await import("../lib/tools/list-inventory");
      expect(mod.spec.name).toBe("list_inventory");
      expect(mod.spec.description).toMatch(/inventory/i);
      expect(mod.spec.inputSchema.safeParse({}).success).toBe(true);
      expect(mod.spec.inputSchema.safeParse({ page: 1, per_page: 20 }).success).toBe(true);
    });
  });
  ...
  describe("happy path", () => {
    it("returns paginated inventory list", async () => {
      server.use(
        http.get("https://platform-api.quiqup.com/api/fulfilment/inventory", () =>
          HttpResponse.json({ items: [{ sku: "MARKER-SKU" }] }),
        ),
      );
      const mod = await import("../lib/tools/list-inventory");
      ...
      expect(first.text).toContain("MARKER-SKU");
    });
  });
});
