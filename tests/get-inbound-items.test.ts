...
describe("get_inbound_items", () => {
  beforeEach(() => vi.clearAllMocks());

  describe("registration", () => {
    it("registers under the expected name with required input schema", async () => {
      const mod = await import("../lib/tools/get-inbound-items");
      expect(mod.spec.name).toBe("get_inbound_items");
  ...
  describe("happy path", () => {
    it("returns inbound items via plural path", async () => {
      server.use(
        http.get(
          "https://platform-api.quiqup.com/api/fulfilment/inbounds/ib1/items",
          () => HttpResponse.json({ items: [{ sku: "item-marker-5" }] }),
        ),
      );
      const mod = await import("../lib/tools/get-inbound-items");
      ...
      expect(first.text).toContain("item-marker-5");
    });
  });
});
